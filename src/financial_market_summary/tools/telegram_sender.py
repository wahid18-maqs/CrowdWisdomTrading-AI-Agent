        ...
        try:
            ...
            try:
                ...
                if screenshots_dir.exists():
                    # Match both chart_*.png and img_*.png files
                    screenshots = list(screenshots_dir.glob("chart_*.png")) + list(screenshots_dir.glob("img_*.png"))
                    if screenshots:
            ...
            # Use the translated_caption that was already extracted from Message 1
            # This contains the engaging hook generated by the AI agent
            if translated_caption:
                final_caption = translated_caption
                logger.info(f"{language}: Using engaging hook from Message 1 as caption")
                logger.info(f"Final caption ({language}): {final_caption[:150]}...")
            else:
                # Fallback: use AI description if Message 1 caption is missing
                logger.warning(f"Message 1 caption not found, falling back to AI description")
                if english_ai_description:
                    if language.lower() == 'english':
                        final_caption = english_ai_description
                        logger.info(f"English: Using AI description as fallback caption")
                    else:
                        ...
                        logger.info(f"{language}: Using translated AI description as fallback caption")
                else:
                    logger.warning(f"No caption available for {language}, will skip image message")
                    final_caption = None
